 ...
 * 当前仓库中的包根目录路径
 */
export const packagesRoot = resolve(projectRoot, 'src');

/**
 * 当前仓库中的 demo 根目录路径
 */
export const playgroundsRoot = resolve(projectRoot, 'demo');

/**
 * 客户端包根目录路径，基于项目根目录下的 src/client 目录
 */
export const clientRoot = resolve(packagesRoot, 'client');

/**
 * 共享代码包根目录路径，基于项目根目录下的 src/shared 目录
 */
export const sharedRoot = resolve(packagesRoot, 'shared');

/**
 * Rollup 包根目录路径，保留与 upstream 脚本一致的常量定义
 */
export const rollupRoot = resolve(packagesRoot, 'rollup');

/**
 * Vite 包根目录路径，基于项目根目录下的 src/vite 目录
 */
export const viteRoot = resolve(packagesRoot, 'vite');

/**
 * Webpack 包根目录路径，保留与 upstream 脚本一致的常量定义
 */
export const webpackRoot = resolve(packagesRoot, 'webpack');

/**
 * 所有 package 目录列表，读取项目根目录下 src 文件夹中的内容
 */
export const packages = readdirSync(resolve(projectRoot, 'src'));

/**
 * 所有 demo 目录列表
 */
export const playgrounds = ['demo'];
